...
public class MakeClassImmutableVisitor extends ASTVisitor {
	...
	public MakeClassImmutableVisitor(
			MakeImmutableRefactoring makeImmutableRefactoring,
			ICompilationUnit unit, ASTRewrite rewriter,
			ClassMutatorAnalysis mutatorAnalysis, RewriteUtil rewriteUtil,
		...
		if (doesParentBindToTargetClass(methodDecl)
			...
			if (!fieldAssignments.isEmpty()) {
				...
				for (SimpleName simpleName : fieldAssignments) {
					Assignment fieldAssignment = (Assignment) ASTNodes
							.getParent(simpleName, Assignment.class);
					...
					for (FieldDeclaration field : fields) {
						...
						for (Object fragmentObject : fragments) {
							...
							if (fragment.getName().getIdentifier().equals(
								...
								Expression assignValue = (Expression) ASTNode
										.copySubtree(astRoot, fieldAssignment
												.getRightHandSide());
								arguments.add(assignValue);
							} else {
								FieldAccess fieldAccessExpression = astRoot
										.newFieldAccess();
								fieldAccessExpression.setExpression(rewriteUtil
										.getThisSimpleName());

								SimpleName fieldName = (SimpleName) ASTNode
										.copySubtree(astRoot, fragment
												.getName());
					...
					Expression mutateExpression = rewriteUtil
							.createMutateExpression(classType, arguments);

					if (fieldAssignment.getParent() instanceof ExpressionStatement) {
						rewriter.replace(fieldAssignment, mutateExpression,
								editGroup);
					}
				}

				/* Replace void return type with an object of this class' type */
				SimpleType returnType = (SimpleType) ASTNode.copySubtree(
						astRoot, classType);
				...
				rewriter.getListRewrite(methodBody, Block.STATEMENTS_PROPERTY)
						.insertLast(returnStatement, editGroup);
	...
	private boolean doesParentBindToTargetClass(MethodDeclaration methodDecl) {
		TypeDeclaration declaringClass = (TypeDeclaration) ASTNodes.getParent(
				methodDecl, TypeDeclaration.class);
		return Bindings.equals(declaringClass.resolveBinding(), refactoring
				.getTargetBinding());
	}
		...
		if (doesParentBindToTargetClass(fieldDecl)) {
			...
			if (!Flags.isFinal(fieldDecl.getModifiers())) {
				TextEditGroup gd = new TextEditGroup(
						"change field to final and add initializer(s)");

				int finalModifiers = fieldDecl.getModifiers()
						| ModifierKeyword.FINAL_KEYWORD.toFlagValue();
				ModifierRewrite.create(rewriter, fieldDecl).setModifiers(
						finalModifiers, gd);

				// Add initializers to the fragments that do not have them as
				// they are required for final variables
				...
				for (Object obj : fragments) {
					...
					if (frag != null && frag.getInitializer() == null) {
						...
						// If the field is already initialized in a constructor
						// or we have mutators (in which case we
						// will have made new constructors) we can't initialize
						// it a second time at the
						// declaration point
						if (!mutatorAnalysis.hasMutators()
								&& !isFieldInitializedInConstructor(
										fieldDeclType, frag, gd)) {

							// Add initializer
							Expression initializer = rewriteUtil
									.createDefaultInitializer(fieldDeclType);
							if (initializer == null)
								continue;

							VariableDeclarationFragment newFrag = (VariableDeclarationFragment) ASTNode
									.copySubtree(astRoot, frag);
							newFrag.setInitializer(initializer);
							rewriter.replace(frag, newFrag, gd);
						}
					} else {
						// If we have added a constructor we must make sure the
						// field is not initialized in the declaration
						if (mutatorAnalysis.hasMutators()) {
							VariableDeclarationFragment newFrag = (VariableDeclarationFragment) ASTNode
									.copySubtree(astRoot, frag);
			...
			if (!Flags.isPrivate(fieldDecl.getModifiers())) {
				...
				int privateModifiers = fieldDecl.getModifiers()
						| ModifierKeyword.PRIVATE_KEYWORD.toFlagValue();
				ModifierRewrite.create(rewriter, fieldDecl).setModifiers(
						privateModifiers, gd2);
	...
	@SuppressWarnings("unchecked")
	private boolean isFieldInitializedInConstructor(Type fieldDeclType,
			VariableDeclarationFragment frag, TextEditGroup gd) {
		boolean isInitializedInConstructor = false;

		// Get the class of the variable (The grandparent of a field/fragment is
		// always its class)
		TypeDeclaration parentClass = (TypeDeclaration) frag.getParent()
				.getParent();
		assert parentClass.isInterface() == false; // Interfaces can't have
		// non-static fields

		// Go through all the constructors and check whether they initialize the
		// variable
		...
		try {
			...
			// We take advantage of the fact that the method lists of both IType
			// and TypeDeclaration are ordered if
			// it is a source file (which are the only files we transform) to
			// build a list of the MethodDeclarations
			...
			if (isInitializedInConstructor) {
				...
				// At least one constructor initializes the variable so it won't
				// be initialized in the declaration
				// and we must add initialization statements to any constructors
				// that lack it
				for (MethodDeclaration methodDecl : methodDeclsWithoutInitialization) {

					// Create an assignment of the default value to frag in
					// methodDecl
					Expression initializer = rewriteUtil
							.createDefaultInitializer(fieldDeclType);
					ExpressionStatement assignStmt = rewriteUtil
							.createFieldAssignmentStatement(fieldDeclType,
									frag, initializer);

					// Add the assign statement to the method body
					rewriter.getListRewrite(methodDecl.getBody(),
							Block.STATEMENTS_PROPERTY).insertLast(assignStmt,
							gd);
				}
	...
	private List getWritesToFieldInMethod(IField field, IMethod method)
		...
		IJavaSearchScope scope = SearchEngine
				.createJavaSearchScope(new IJavaElement[] { method });
	...
	private boolean doesParentBindToTargetClass(FieldDeclaration fieldDecl) {
		...
		if (parent != null) {
			...
			return Bindings.equals(typeDecl.resolveBinding(), refactoring
					.getTargetBinding());
		}
		return false;
	}
